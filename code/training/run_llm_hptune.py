...
HP_CONFIGS = {
    "default": {
        "n_steps": 2048, "ent_coef": 0.01, "learning_rate": 2.5e-4,
        "batch_size": 128, "net_arch": [64, 64],
        "label": "A: Default (64x64, lr=2.5e-4, n=2048)",
    },
    "deep": {
        "n_steps": 2048, "ent_coef": 0.01, "learning_rate": 1e-4,
        "batch_size": 256, "net_arch": [128, 128, 64],
        "label": "B: Deep (128x128x64, lr=1e-4, n=2048)",
    },
    "explore": {
        "n_steps": 4096, "ent_coef": 0.05, "learning_rate": 3e-4,
        "batch_size": 256, "net_arch": [64, 64],
        "label": "C: Explore (64x64, lr=3e-4, n=4096, ent=0.05)",
    },
}

# ── Output ──
OUTPUT_DIR = OUR_CODE / "llm_hptune_results"
...
class NormalizedObsWrapper(gym.Wrapper):
    """
    Selective normalization: z-score cash/price/holdings + 25x NLP upscale.
    Works well with dollar_delta reward.
    """
    def __init__(self, env, obs_mean=None, obs_std=None, nlp_scale=25.0, eps=1e-8):
        ...
        # Indices to z-score: cash (0), price (1), holdings (2)
        self.zscore_indices = list(range(1 + 2 * stock_dim))
        self.obs_mean = obs_mean
        self.obs_std = obs_std

    def compute_stats_from_data(self, train_df):
        """Compute z-score stats from training data for cash/price/holdings."""
    ...
    def _transform(self, obs):
        ...
        # Z-score normalize cash, price, holdings
...
def train_single(llm_config, ticker, seed, output_dir, hp_name="default", hp=None):
    ...
    if hp is None:
        hp = HP_CONFIGS["default"]
    run_name = f"{hp_name}_{llm_config}_{ticker}_seed{seed}"
    ...
    try:
        ...
        model = PPO(
            "MlpPolicy", train_env,
            n_steps=hp["n_steps"], ent_coef=hp["ent_coef"],
...
def evaluate_ensemble(llm_config, ticker, output_dir, hp_name="default"):
    """
...
def run_phase1(output_dir, hp_name="default", hp=None):
    """Phase 1: Train all 250 models."""
    if hp is None:
        hp = HP_CONFIGS[hp_name]
    print("=" * 70)
    print(f"PHASE 1: TRAINING (250 runs, hp={hp_name})")
    print("=" * 70)
    ...
    for llm_config in LLM_CONFIGS:
        for ticker in TICKERS:
            for seed in SEEDS:
                ...
                print(f"\n[{done}/{total}] ", end="")
                is_active, metrics = train_single(llm_config, ticker, seed, output_dir, hp_name=hp_name, hp=hp)
...
def run_phase2(output_dir, hp_name="default"):
    """Phase 2: Ensemble evaluation."""
    print("\n" + "=" * 70)
    print(f"PHASE 2: ENSEMBLE EVALUATION (hp={hp_name})")
    print("=" * 70)

    all_ensemble_metrics = []
    for llm_config in LLM_CONFIGS:
        for ticker in TICKERS:
            m = evaluate_ensemble(llm_config, ticker, output_dir, hp_name=hp_name)
            m["hp_name"] = hp_name
            all_ensemble_metrics.append(m)

    results_df = pd.DataFrame(all_ensemble_metrics)
    results_df.to_csv(output_dir / f"ensemble_results_{hp_name}.csv", index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hp", type=str, default="default", help="Hyperparameter config: default, deep, explore")
    args = parser.parse_args()

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    hp_name = args.hp
    hp = HP_CONFIGS.get(hp_name, HP_CONFIGS["default"])
    ...
    if args.config or args.ticker or args.seed:
        ...
        train_single(cfg, t, s, OUTPUT_DIR, hp_name=hp_name, hp=hp)
    elif args.phase == 1:
        training_metrics = run_phase1(OUTPUT_DIR, hp_name=hp_name, hp=hp)
    elif args.phase == 2:
        ensemble_metrics = run_phase2(OUTPUT_DIR, hp_name=hp_name)
    else:
        training_metrics = run_phase1(OUTPUT_DIR, hp_name=hp_name, hp=hp)
        ensemble_metrics = run_phase2(OUTPUT_DIR, hp_name=hp_name)
